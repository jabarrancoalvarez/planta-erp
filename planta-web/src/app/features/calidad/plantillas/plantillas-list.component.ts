...
import { CalidadService, PlantillaListDto } from '../../../core/services/calidad.service';
  ...
  template: `
    <div class="page">
      <div class="page__header">
        <div>
          <h1 class="page__title">Plantillas de Inspeccion</h1>
          <p class="page__subtitle">Definicion de criterios de calidad</p>
        </div>
        <button class="btn-primary" (click)="showForm.set(true)">+ Nueva Plantilla</button>
      </div>
      ...
      @if (loading()) {
        <div class="loading-state">Cargando plantillas...</div>
      } @else if (error()) {
    ...
    @if (showForm()) {
      <div class="form-overlay" (click)="closeForm()">
        <div class="form-dialog" (click)="$event.stopPropagation()">
          <h2 class="form-dialog__title">Nueva Plantilla de Inspeccion</h2>
          <form [formGroup]="form" (ngSubmit)="onSubmit()">
            ...
            <div class="form-group">
              <label class="form-label">Origen Inspeccion *</label>
              <select class="form-select" formControlName="origenInspeccion">
...
export class PlantillasListComponent implements OnInit {
  ...
  readonly items = signal<PlantillaListDto[]>([]);
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    descripcion: [''],
    origenInspeccion: ['', Validators.required],
  });
  ...
  goToDetail(id: string): void { this.router.navigate(['/app/calidad/plantillas', id]); }
  ...
  onSubmit(): void {
    ...
    this.svc.createPlantilla({
      nombre: v.nombre,
      descripcion: v.descripcion || undefined,
      origenInspeccion: v.origenInspeccion,
    }).subscribe({
      next: () => { this.notify.success('Plantilla creada correctamente'); this.closeForm(); this.submitting.set(false); this.load(); },
      error: (err) => { this.notify.error(err?.error?.message ?? 'Error al crear plantilla'); this.submitting.set(false); },
    });
  }
  ...
  private load(): void {
    ...
    this.error.set(null);
    this.svc.listPlantillas(this.currentSearch || undefined, this.currentPage()).subscribe({
      ...
      error: (err) => { this.error.set(err?.error?.message ?? 'Error al cargar plantillas'); this.loading.set(false); },
    });
  }
}
